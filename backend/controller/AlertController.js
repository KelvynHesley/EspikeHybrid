...
router.post("/", authenticate, async (req, res) => {
  try {
    ...
    res
      .status(400)
      .json({ message: "Erro ao criar alerta", error: error.message });
  }
});
...
router.delete("/:id", authenticate, authorize(["mod"]), async (req, res) => {
  try {
    ...
    res
      .status(400)
      .json({ message: "Erro ao deletar alerta", error: error.message });
  }
});
