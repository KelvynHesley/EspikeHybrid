...
router.get("/me", authenticate, async (req, res) => {
  try {
    // Correção 1: Use UserService (igual ao import)
    // Correção 2: Use getUserById (método que existe) passando o ID do token
...
router.post("/", async (req, res) => {
  try {
    ...
    if (
      error.message &&
      error.message.includes("Já existe um usuário com esse email")
    ) {
    ...
    res
      .status(500)
      .json({ message: "Ocorreu um erro ao adicionar o usuário." + error });
  }
});
